...
-opaque queue(EZS) :: {queue, list(EZS), list(EZS)}.
...
-spec from_list(list(EZV)) -> queue(EZV).
from_list(List) ->
    {queue, [], List}.

-file("/Users/louis/src/gleam/stdlib/src/gleam/queue.gleam", 54).
-spec to_list(queue(EZY)) -> list(EZY).
to_list(Queue) ->
...
-spec push_back(queue(FAF), FAF) -> queue(FAF).
...
-spec push_front(queue(FAI), FAI) -> queue(FAI).
...
-spec pop_back(queue(FAL)) -> {ok, {FAL, queue(FAL)}} | {error, nil}.
pop_back(Queue) ->
    case Queue of
        {queue, [], []} ->
            {error, nil};
...
-spec pop_front(queue(FAQ)) -> {ok, {FAQ, queue(FAQ)}} | {error, nil}.
pop_front(Queue) ->
    case Queue of
        {queue, [], []} ->
            {error, nil};
...
-spec reverse(queue(FAV)) -> queue(FAV).
reverse(Queue) ->
...
-spec is_logically_equal(queue(FAY), queue(FAY), fun((FAY, FAY) -> boolean())) -> boolean().
...
-spec is_equal(queue(FBG), queue(FBG)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),
